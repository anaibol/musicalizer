...
(($) ->
  ...
  init = (t, options) ->
    ...
    t.addClass(SIGNATURE).attr(ID, id).before "<div class=\"grips\"/>" #the grips container object is added. Signature class forces table rendering in fixed-layout mode to prevent column's min-width
  ...
  createGrips = (t) ->
    ...
    th.each (i) -> #iterate through the table column headers
      c = $(this).add(t.find('>.tbody > thead > tr > th.' + $(this).attr('class'))) #jquery wrap for the current column
      g = $(t.gc.append("<div class=\"grip\"></div>")[0].lastChild) #add the visual node to be used as grip
      ...
      if i < t.ln - 1 #bind the mousedown event to start dragging
        ...
        g.addClass("lastGrip").removeClass "grip"
  ...
  $.fn.extend colResizable: (options) ->
    defaults =      
      #attributes:
      draggingClass: 'dragging' #css-class used when a grip is being dragged (for visual feedback purposes)
      ...
      postbackSafe: true #when it is enabled, table layout can persist after postback. It requires browsers with sessionStorage support (it can be emulated with sessionStorage.js). Some browsers only
